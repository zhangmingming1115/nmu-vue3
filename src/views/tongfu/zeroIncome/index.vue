<template>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      ...
      <el-table-column label="累计开票(不含税)" align="center" prop="totalInvoicedNoTax"/>
      <el-table-column label="累计收款(含税)" align="center" prop="totalReceivedTax"/>
      <el-table-column label="可进收" align="center" prop="zeroIncome"/>
      <el-table-column label="计划毛利率(%)" align="center" prop="plannedGrossProfitRate"/>
...
const getList = async () => {
  loading.value = true
  try {
    const data = await TongFuProjectQueryApi.zeroIncome(queryParams)
...
const handleExport = async () => {
  try {
    ...
    const data = await TongFuProjectQueryApi.zeroIncomeExport(queryParams)
    download.excel(data, '零进取.xls')
  } catch {
  } finally {
