<template>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      ...
      <el-table-column label="计划收入(含税)" align="center" prop="plannedIncomeTax"/>
      <el-table-column label="计划收入(不含税)" align="center" prop="plannedIncomeNoTax"/>
      ...
      <el-table-column label="外包/合作费成本" align="center" prop="outsourcingCost"/>
      <el-table-column label="实际外包成本" align="center" prop="actualOutsourcingCost"/>
      <el-table-column label="累计收票(含税)" align="center" prop="totalInvoiceReceivedTax"/>
      <el-table-column label="可分包" align="center" prop="subcontractable"/>
...
const getList = async () => {
  loading.value = true
  try {
    const data = await TongFuProjectQueryApi.subcontractable(queryParams)
...
const handleExport = async () => {
  try {
    ...
    const data = await TongFuProjectQueryApi.subcontractableExport(queryParams)
    download.excel(data, '可分包.xls')
  } catch {
  } finally {
