<template>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      ...
      <el-table-column label="累计付款(含税)" align="center" prop="totalPaidTax"/>
      <el-table-column label="累计收款(含税)" align="center" prop="totalReceivedTax"/>

      <el-table-column label="累计开票(含税)" align="center" prop="totalInvoicedTax"/>
      <el-table-column label="累计收票(含税)" align="center" prop="totalInvoiceReceivedTax"/>
      <el-table-column label="可直接付款" align="center" prop="directPayable"/>
...
const getList = async () => {
  loading.value = true
  try {
    const data = await TongFuProjectQueryApi.directPayable(queryParams)
...
const handleExport = async () => {
  try {
    ...
    const data = await TongFuProjectQueryApi.directPayableExport(queryParams)
    download.excel(data, '可付款.xls')
  } catch {
  } finally {
