<template>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      ...
      <el-table-column label="可进取(不含税)" align="center" prop="incomeReceivable"/>
...
const getList = async () => {
  loading.value = true
  try {
    const data = await TongFuProjectQueryApi.incomeReceivable(queryParams)
...
const handleExport = async () => {
  try {
    ...
    const data = await TongFuProjectQueryApi.incomeReceivableExport(queryParams)
    download.excel(data, '可进取.xls')
  } catch {
  } finally {
