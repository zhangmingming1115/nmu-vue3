<template>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      ...
      <el-table-column label="可开票不含税" align="center" prop="invoicingAvailable"/>
...
const getList = async () => {
  loading.value = true
  try {
    const data = await TongFuProjectQueryApi.invoicingAvailable(queryParams)
...
const handleExport = async () => {
  try {
    ...
    const data = await TongFuProjectQueryApi.invoicingAvailableExport(queryParams)
    download.excel(data, '可开票.xls')
  } catch {
  } finally {
