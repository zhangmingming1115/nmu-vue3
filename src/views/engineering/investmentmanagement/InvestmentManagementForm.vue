      ...
      <el-row>
        <el-col :span="8">
          <el-form-item label="立项批复日期" prop="approvalDateOfProjectInitiation">
            <el-date-picker
              v-model="formData.approvalDateOfProjectInitiation"
              type="date" value-format="x" placeholder="选择立项批复日期"
        ...
        <el-col :span="8">
          <el-form-item label="实际开工日期" prop="actualStartDate">
            <el-date-picker
              ...
              type="date" value-format="x" placeholder="选择实际开工日期"
              :readonly="formType === 'detail'" :disabled="formType === 'detail'"/>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="实际初验日期" prop="actualPreliminaryAcceptanceDate">
            <el-date-picker v-model="formData.actualPreliminaryAcceptanceDate"
                            type="date" value-format="x" placeholder="选择实际初验日期"/>
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <el-row>
        <el-col :span="8">
          <el-form-item label="提交预转固日期" prop="dateOfPreFixedAssetApplication">
            <el-date-picker
              v-model="formData.dateOfPreFixedAssetApplication"
              type="date" value-format="x" placeholder="选择提交预转固日期"/>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="预转固日期" prop="preFixedAssetDate">
            <el-date-picker
              v-model="formData.preFixedAssetDate"
              type="date" value-format="x" placeholder="选择预转固日期"/>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="实际终验日期" prop="actualFinalAcceptanceDate">
            <el-date-picker
              v-model="formData.actualFinalAcceptanceDate"
              type="date" value-format="x" placeholder="选择实际终验日期"/>
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <el-row>
        <el-col :span="8">
          <el-form-item label="提交决算／转固日期" prop="dateOfFinalAccountFixedAssetTransfer">
            <el-date-picker v-model="formData.dateOfFinalAccountFixedAssetTransfer"
                            type="date" value-format="x" placeholder="选择提交决算／转固日期"/>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="转固日期" prop="fixedAssetTransferDate">
            <el-date-picker v-model="formData.fixedAssetTransferDate"
                            type="date" value-format="x" placeholder="选择转固日期"/>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="工程关闭日期" prop="projectCloseDate">
            <el-date-picker v-model="formData.projectCloseDate"
                            type="date" value-format="x" placeholder="选择工程关闭日期"/>
          </el-form-item>
        </el-col>
      </el-row>
...
/** 工程投资管理 表单 */
defineOptions({ name: 'InvestmentManagementForm' })
...
const open = async (type: 'create' | 'update' | 'detail', id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await InvestmentManagementApi.getInvestmentManagement(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as InvestmentManagementVO
    if (formType.value === 'create') {
      await InvestmentManagementApi.createInvestmentManagement(data)
      message.success(t('common.createSuccess'))
    } else {
      await InvestmentManagementApi.updateInvestmentManagement(data)
